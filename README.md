...
You can create a function list, represented as a `Fist`, in from a hard-coded set of parameters, from an existing `Fist`,
or from a Linq `Range`.


##### From hard-coded list of elements
...
`Fist`s that are always lazy-evaluated. For example, a Linq `Range` used to create a `Fist` is only
traversed as values are needed.

Similarly, a mapped `Fist` only runs the mapping method as values are read from the mapped `Fist`, a call to
`any()` stops evaluating as soon as the first `true` is detected, and `all()` stops at the first `false`.

When writing code, never depend on a side-effect of element evalution. Outside of well-documented cases such as
`println()`, treat all function calls as [pure](https://en.wikipedia.org/wiki/Pure_function).

Keep in mind that while most functions have minimal evaluation overhead, some functions, such as `reverse()`, may
need to eagerly traverse the entire list. Be cognizant of what you are asking for when you call a function.
...
At least [one attempt to make it better](https://github.com/hazzik/Tail.Fody) exists, but it seems to be outdated. And when I made the minor modifications to get it compiling in dotnet 3.0, it didn't detect most of the tail recursion calls. In particular, the IL emitted for tail calls embedded within C# 8.0 [`switch` expressions](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-8#switch-expressions) weren't detected.
